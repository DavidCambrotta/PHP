...
function str_len(string $s): int {
    ...
    if (function_exists('grapheme_strlen')) { $n = grapheme_strlen($s); if ($n !== false) return $n; }
    return strlen($s);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (str_len($old['name']) < 2)   $errors['name'] = 'Name must be at least 2 characters.';
    elseif (str_len($old['name']) > 60)  $errors['name'] = 'Name must be at most 60 characters.';
    elseif (!preg_match("/^[\p{L}\p{M}'\- ]+$/u", $old['name'])) $errors['name'] = 'Use letters, spaces, apostrophes, or hyphens only.';

    if ($old['email'] === '')        $errors['email'] = 'Email is required.';
    elseif (!filter_var($old['email'], FILTER_VALIDATE_EMAIL)) $errors['email'] = 'Enter a valid email address.';

    if ($old['subject'] === '')      $errors['subject'] = 'Subject is required.';
    elseif (str_len($old['subject']) < 3)  $errors['subject'] = 'Subject must be at least 3 characters.';
    elseif (str_len($old['subject']) > 100)$errors['subject'] = 'Subject must be at most 100 characters.';
    ...
    if (!$errors) {
        try {
            ...
            $warningMessage = 'Database error: ' . h($e->getMessage());
        }
    }
}
?>
...
<title>Contact Form (PHP + SQLite)</title>
<style>
  ...
  <div class="card">
    ...
    <form method="post" action="" novalidate>
      ...
      <div>
        ...
        <?php if (!empty($errors['subject'])): ?><div class="field-error"><?= h($errors['subject']) ?></div><?php endif; ?>
      </div>
    ...
    <p class="admin-link"><a href="admin.php">Admin: view submissions →</a></p>
  </div>
</body>
</html>
